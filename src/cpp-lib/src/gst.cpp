...
namespace Gst {
    ...
    std::string pad_caps_type(GstPad* pad)
    {
        ...
        if(caps){
            ...
            if(caps_struct){
                pad_type = string(gst_structure_get_name(caps_struct));
            }
            gst_caps_unref(caps);
        }
        return pad_type;
    ...
    {
        CHECK_NULL_RET(element, "Can't get name, Element ", "");
    ...
    bool pad_link_element_request(GstPad* pad, GstElement* element, const string& e_pad_name)
    {
        ...
        CHECK_NULL_RET(element, "Not link, Element", false);
            ...
            GstElement* sink, const char* sink_name)
    {
        ...
        if(int err = gst_pad_link(pad_src, pad_sink); err != GST_PAD_LINK_OK){
            LOG(error) << "Can'n link err: " << err << ", " <<  
    ...
    int on_bus_message (GstBus * , GstMessage * message, gpointer user_data)
        {
            ...
            */
            // TODO: got elements error to detect stream corruption
    ...
    bool demux_pad_link_to_muxer(
        ...
        if(parse != nullptr){
            ...
            if(queue_buffer_zero){
                Gst::set_max_queue_time(queue, 0);  
            } 
    ...
    void queue_overrun_quit(GstElement* queue, gpointer user_data)
    {
    ...
    void queue_guard(GMainLoop* loop, GstElement* queue, int sec)
    {
        g_object_set(queue,
                ...
                nullptr);
        g_signal_connect(queue, "overrun", G_CALLBACK( queue_overrun_quit ), loop);
    }
}
